# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.5.0] - 2026-06-07
### Added
- Added custom premium dark-mode scrollbars across the application.
- Completed Phase 4 (Polish) to finalize the initial Minimum Viable Product (MVP) for Cortex.

## [0.4.0] - 2026-06-07
### Added
- Implemented `MemoryTimeline.tsx` as a sleek, slide-out right panel to view chronological events.
- Created `TimelineItem.tsx` with color-coded node indicators.
- Updated `App.tsx` overlay to include a stateful "View Timeline" toggle button.

## [0.3.0] - 2026-06-07
### Added
- Integrated `@xyflow/react` (React Flow) as the core node-graph engine.
- Created custom `FactNode.tsx` component with premium dark-mode styling for memory facts.
- Implemented `MemoryGraph.tsx` to visualize the interconnected AI memory nodes.
- Redesigned `App.tsx` layout to include a full-screen interactive canvas and robust sidebar.

## [0.2.0] - 2026-06-07
### Added
- Installed and configured TailwindCSS v4 inside the Vite pipeline.
- Created the initial dark-mode UI shell in `src/App.tsx` for Cortex.

## [0.1.0] - 2026-06-07
### Added
- Scaffolded Tauri v2 + React + TypeScript + Vite boilerplate.
- Initialized core application directories (`src/` for frontend, `src-tauri/` for backend).

## [0.0.1] - 2026-06-07
### Added
- Initial project structure (`PLAN.md`, `README.md`, `CHANGELOG.md`).
- `.agents` configuration directory for rules and workflows.

[0.5.0]: https://github.com/dhia/cortex/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/dhia/cortex/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/dhia/cortex/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/dhia/cortex/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/dhia/cortex/compare/v0.0.1...v0.1.0
[0.0.1]: https://github.com/dhia/cortex/releases/tag/v0.0.1
